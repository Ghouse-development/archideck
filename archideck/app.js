...
// IC回の状態を計算（gray/blue/yellow/red）
  ...
  if (hasAnyValue) {
    ...
    if (roundIndex > 0) {
      ...
      if (prevStatus !== 'blue' && prevStatus !== 'gray') {
        // 前の回が黄色（途中）でこの回に進んでいる場合、前の回を赤にする判定は別途
      }
    }
    return 'yellow';
  }
...
function generateICRoundCards(project, progressData) {
  ...
  const contentsHtml = IC_ROUNDS.map(round => {
    const roundTasks = round.tasks.map(taskKey => {
      ...
      const showEmailButton = taskDef.has_email_button !== false && hasVendor && !isInternalStatus;
      const emailBtn = showEmailButton ?
        `<button class="task-email-btn" onclick="openEmailFromTask('${projectId}', '${taskKey}')" title="メール作成">📧</button>` : '';
      ...
      const dueDateInput = taskKey === 'ic_final_checklist'
        ? `<input type="date" class="task-due-input" value="${task.due_date || ''}" onchange="updateTaskDueDate('${projectId}', '${taskKey}', this.value)" title="期限を設定">`
        : '';

      const memoInput = taskDef.has_memo
        ? `<input type="text" class="task-memo-input" value="${task.memo || ''}" placeholder="依頼内容を入力" onchange="updateTaskMemo('${projectId}', '${taskKey}', this.value)" >`
        : '';
