...
1. **申請GO位置維持の強化**
   - `updated_at`を明示的に元の値で送信（Supabaseトリガー対策）
   - トリガーによる自動更新を上書きして位置を維持

2. **ICメールボタン表示修正**
   - `IC_EMAIL_BUTTON_DEFAULTS`フォールバック設定追加
   - 設計タスクと同様のシンプルなロジックに変更
   - `IC_MAKER_TASKS`に`ic_iron`（アイアン依頼）追加

3. **変更履歴モーダル修正**
   - `.modal-backdrop`のCSS追加（モーダルが表示されない問題を修正）
   - ユーザー名をメールアドレスからdesignersマスターの名前で表示

4. **DOM警告修正**
   - パスワードフィールド6箇所に`autocomplete`属性追加

5. **設計タスク依頼日バッジ削除**
   - 「仮ポイント指示」などの設計タスク横の日付表示を削除

6. **完了モーダルデバッグログ追加**
   - 設計/ICタスクの完了チェック状況をコンソールに出力
   - 未完了タスク名を表示して問題特定を容易に

**変更ファイル**:
- `archideck/app.js`: ICメール、申請GO、変更履歴、完了モーダル
- `archideck/styles.css`: `.modal-backdrop`追加
- `archideck/index.html`: `autocomplete`属性追加
...
1. **ステータス変更履歴の強化**
   - `saveChangeHistory`関数のエラーハンドリングを強化
   - Supabase insertの戻り値を正しくチェック（.select()を追加）

2. **申請GO位置維持の修正**
   - `updated_at`をDB更新に含めないよう変更（位置変更を防止）
   - ローカル状態でも元の`updated_at`を明示的に保持（originalUpdatedAt）
   - リアルタイム同期のスキップ時間を2秒→5秒に延長

3. **完了モーダルの修正**
   - 完了チェックのトリガー条件を緩和（taskDefが未定義でもチェック実行）

4. **kintone自動同期の即時化**
   - 5秒遅延を削除、初期化完了後即座に実行
   - ログイン・リフレッシュごとに最新データを取得

**変更ファイル**:
- `archideck/app.js`: 729, 2321, 2405, 7064, 13103-13119行
- `CLAUDE.md`: 出荷・完遂契約を改訂版vNextに更新

---

## v4.99.71 (2026-01-17)

### 重大バグ修正: TASK_KEY_MAPPINGの定義位置

**問題**: `TASK_KEY_MAPPING`が7054行目で定義されていたが、フォールバックコード（6219行目）より後だったため、フォールバックが動作しなかった。

**修正内容**:
- `TASK_KEY_MAPPING`を703行目（IC関連定数の直後）に移動
- `migrateProgressDataKeys()`を常に実行するよう変更（マイグレーションスキップ時も実行）
- `autoMigrateICTasks()`内の重複呼び出しを削除
...
**修正した問題**:
1. **ICタスク進捗データ取得**: 旧キー(ic_washroom_1f/2f等)から新キー(ic_washroom等)へのフォールバック対応
2. **進捗データマイグレーション**: 旧キーの進捗データを新キーに自動移行する処理を追加
3. **申請GO位置維持**: markLocalUpdate()を追加してリアルタイム同期による位置変更を防止
4. **ICメールボタン表示**: フォールバック対応により旧データでもメールボタンが正しく表示
5. **完了モーダル**: フォールバック対応により旧データでも完了チェックが正常動作

**技術的変更**:
- ICタスク表示時に`TASK_KEY_MAPPING`を参照して旧キーから進捗データを取得
- `migrateProgressDataKeys()`関数を追加: 洗面・トイレの進捗データを統合キーに移行
- `executeApplicationGo()`に`markLocalUpdate()`を追加
...
- `ic_washroom_1f`/`ic_washroom_2f` → `ic_washroom`（洗面）に統合
- `ic_toilet_1f`/`ic_toilet_2f` → `ic_toilet`（トイレ）に統合
- ICタスク数: 27項目 → 25項目に変更
- 関連定数（IC_MAKER_TASKS等）を更新

### 申請GO後のカード位置維持を修正

- DBトリガーによる`updated_at`自動更新を回避
- `project.updated_at`を明示的に保持して更新
...
- 以下10タスクに対して`has_email_button: true`を常に保証:
  - キッチン・カップボード、お風呂、洗面、トイレ
  - 照明プラン、建具プレゼン、タイルプレゼン
  - カーテン紹介、造作業者紹介、家具見積依頼
- `syncICEmailButtonSettings()`関数を追加
- アプリ起動時に自動実行し、設定が正しくない場合はDBを更新
- マイグレーション済みでも設定が壊れていた場合に自動修復
...
#### ① ステータス変更履歴
- 既存実装済み（`saveChangeHistory`がupdateTaskStateから呼ばれていることを確認）

#### ② ステータス再クリックで選択解除
- 単一選択モードでアクティブなカードをクリックすると「-」に戻る

#### ③ 申請GOで案件位置を変えない
- `updated_at`の更新を削除し、ソート順を維持

#### ④ 間取確定日過ぎ＋未完了で赤カード
- `layout_confirmed_date`が過去で「間取確定」タスクが未完了の場合、カードを赤く表示

#### ⑤ 完了済移動モーダル問題調査
- ICタスクの完了チェックに`has_state`フィルター追加
- デバッグログを追加（設計・IC完了チェック結果を出力）

#### ⑥ ICタスク「会議図面渡し」追加
- `ic_meeting_drawing`をdisplay_order 24に配置（確定図チェックリストの下）
- ステータス: `["-", "送付済"]`（送付済で青色）
- kintone「会議図面渡し日」のバッジ表示対応
- 日付過ぎ＋未完了で赤カード判定追加

### 追加修正

#### メールボタン
- カードヘッダーの「完了」右のメールボタン（quickEmail）を削除
- ICメールボタン表示条件改善: `has_email_button`がtrueのタスクでステータス選択時に表示
...
#### 設計タスク
- **間取確定**: タスク横にkintone日付（間取確定日）を追加
- **変更契約前会議**: タスク横にkintone日付（変更契約前会議日）を追加

#### ICタスク
- **資金計画・引継書確認**: 日付表示を削除（不要）
...
#### 仕様変更
- **申請GO**: 実行しても「完了済み」に移動しないように変更
- **全タスク完了時**: 設計+ICタスクが全て完了したら「完了済みに移動しますか？」と確認

#### UI修正
- モーダルタイトル「確認申請」→「申請GO」に修正
- モーダル説明文・ボタンテキストを適切な内容に修正
- アイコンを📋→🚀に変更
...
#### 新規タスク追加
- **解体後現地確認**: 現地調査の下に追加（「-」「無し」「完了済」）
- **変更契約前会議**: 申請GOの下に追加（「-」「完了済」）
- **仮ポイント指示**: 変更契約前会議の下に追加（「-」「無し」「依頼済」）

#### タスク分解
- **構造依頼** → **構造①**（ラフチェック〜変更契約前会議UP）+ **構造②**（確定図をダンドリワークUP）

#### 色判定調整
- **申請GO**: 完了済でも青色にならない（控えめに）

### ICタスク更新

#### 新規タスク追加
- **変更契約前会議**: 確定図チェックリストの下に追加

#### kintone日付表示
- 変更契約前会議タスクの横にkintone日付を表示（オレンジ色バッジ）
- 資金計画・引継書確認の横に間取確定日を表示

### その他
- メール作成でPCのOutlookアプリを開くように変更（mailto:リンク）
...
#### DBマイグレーション
- `tasks`テーブルを新規作成（Supabase SQL Editor）
- `has_memo`カラムを追加
- RLSポリシーを設定

#### ICタスク26項目を設定
- **メールボタン必須（14項目）**: キッチン・カップボード、お風呂、洗面1階・2階、トイレ1階・2階、照明プラン、建具プレゼン、タイルプレゼン、カーテン紹介、造作業者紹介、家具見積依頼、アイアン依頼、その他見積依頼
- **メールボタン不要（12項目）**: 資金計画・引継書確認、仕様書作成、長期資料送付、外構への打合せ依頼、実施図、外装プレゼン、内装プレゼン、アイアンプレゼン、確定図チェックリスト、OP見積チェック、会議後確認事項送付、確定図承認

#### UI更新
- ICタスク更新ボタンのラベルを「26項目（メールボタン設定含む）」に修正
...
#### 色判定の改善
- **メーカー選択タスク**（キッチン・お風呂・洗面・トイレ・照明）: メーカーを選ぶと青色に表示
- **依頼系タスク**（アイアンプレゼン・タイルプレゼン・外構・カーテン・造作・家具）: 「無し」「保存済」は青色、「依頼済」は黄色

#### 自動アーカイブ機能
- 確定図承認が「ダンドリワーク保存済」かつ全タスク完了で自動的にアーカイブ確認ダイアログを表示

#### 完了済みフォルダの部署分け
- サイドバーに「設計」「IC」別の完了済みフォルダを追加
- 間取確定日の有無で自動振り分け

#### タスク更新（要SQL実行: v14_ic_task_updates.sql）
- **タスク名変更**:
  - 建具プレゼン依頼 → 建具プレゼン
  - 外装プレゼン作成 → 外装プレゼン
  - 内装プレゼン作成 → 内装プレゼン
  - タイルプレゼン作成 → タイルプレゼン
- **新規タスク追加**: アイアンプレゼン（タイルプレゼンの上）
- **ステータス統一**: タイルプレゼン等を「-/無し/依頼済/保存済」に
...
- 未使用のIC回別コードを削除
  - `IC_ROUNDS`定数
  - `checkICCompletionAndArchive`関数
  - `getICRoundStatus`関数
  - `generateICRoundCards`関数
  - `toggleICRound`関数
- 約200行のデッドコードを削除
...
- IC回別カード表示（1回目〜5回目）を廃止
- 設計と同じグリッド形式でICタスクを一覧表示
- よりシンプルで見やすいレイアウトに統一
...
- **案件管理画面**: 「削除」ボタンを削除（誤操作防止）
- **案件編集画面**: 「テンプレート保存」「テンプレート適用」ボタンを削除
- **メモ機能**: 案件カードからメモボタンと関連機能を削除

### カレンダー機能強化

- **会議図面渡し日**: kintoneフィールド「日付_IC_会議_図面_渡し日_予定手入力」をカレンダーに表示
- **登録タスクの期限**: project_tasksに登録したタスクの期限をカレンダーに表示
- **表示項目整理**: 「面積チェック」「evoltz」をカレンダー表示から除外

### 申請GO機能改善

- **条件の明示表示**: 申請GO未達時、各タスクの現在状態と必要状態を一覧表示
- **完了条件の厳格化**: 申請GO実行時、未完了タスクや登録タスクが残っていれば完了済みに移動しない

### 議事録表示改善

- 「0回目」「０回目」という名称を「第０回目打合せ」に変換して表示

### データベース

- projectsテーブルに`meeting_drawing_date`カラムを追加

---

## v4.99.44 (2026-01-11)

### タスク-業者紐づけを正しいシステムに統一

- `openEmailFromTask`関数とhasVendorチェックをV2システムから旧システムに変更
- taskMappings + vendorsによる紐づけに統一
- evoltz依頼などの業者紐づけが正しく認識されるようになった

---

## v4.99.43 (2026-01-11)

### タスク-業者マッピングのデバッグ機能追加

- `openEmailFromTask`にデバッグログを追加
- taskKey、templateId、マッチした業者数を出力

---

## v4.99.42 (2026-01-11)

### CSS/JSファイルのパスを絶対パスに修正
...
### taskMappingsによるタスク-業者紐付けを復活

- V2システムから旧システムへ戻す
- taskMappingsオブジェクトでタスクとテンプレートIDをマッピング
...
### 業者・依頼メール管理をタスクベースに刷新

- タスクごとに業者とメールテンプレートを管理する新システム
...
### 業者追加が全テンプレートで動作するよう修正

- 一部テンプレートで業者追加が動作しない問題を修正
...
- `loadOrganization()`内の各Supabaseクエリに5秒タイムアウト追加
- 12のload関数すべてに開始・完了のデバッグ出力を追加
- どの関数がハングしているか黄色いオーバーレイで確認可能
...
**問題**
- ログイン成功後、`init()`関数内でSupabaseクエリがハングする
- 画面は表示されるがデータ読み込みが完了しない

**対応内容**
- `loadOrganization()`に10秒タイムアウトを追加
- データ読み込み（12のload関数）に30秒タイムアウトを追加
- `init()`関数の各ステップにデバッグ出力を追加
- `loadOrganization()`内の各Supabaseクエリにデバッグ出力を追加
- タイムアウト時はエラーメッセージを表示し、アプリは継続利用可能

**デバッグ出力**
画面上部の黄色いオーバーレイに以下のステップが表示される：
- `init: 開始` → `init: 基本初期化完了`
- `init: loadOrganization開始` → `loadOrg: memberData取得開始` → ...
- `init: loadOrganization完了` → `init: データ読み込み開始`
- `init: データ読み込み完了` → ... → `init: 完了`
...
**根本原因の特定**
- `session`オブジェクトは存在するが`session.user`が`null`のケースがある
- `session.user.email`へのアクセスでTypeErrorが発生
- try-catchの外で発生するためキャッチされない

**修正内容**
- `if (session)` → `if (session && session.user)` に変更
- onAuthStateChangeハンドラーも同様に修正
...
**kintone設定のDB保存**
- フィールドマッピングをDBに保存（`field_mappings_json`カラム）
- Clear site data実行後も設定が消えなくなった
- localStorageはフォールバックとして維持

**手動kintone同期ボタン**
- 案件管理画面に「🔄 kintone」ボタンを追加
- ワンクリックでkintoneと同期可能
...
- `designers`テーブルに`auth_confirmed`カラムを追加
- ログイン成功時に`auth_confirmed = true`に自動更新
- ログイン済みの担当者には「📧 招待」ボタンを非表示
- ログイン済みの担当者名に✓マークを表示
...
- 担当管理画面に「📧 招待」ボタンを追加
- 既存担当者にパスワード設定メールを再送信可能に
- Supabase Authにユーザーが存在しない場合は自動作成
