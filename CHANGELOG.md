...
**原因**
- 認証処理中にエラーが発生するとログインコンテナが隠れた状態でメインコンテナが表示されないまま処理が中断

**修正内容**
- `checkAuth()`関数に包括的なtry-catch追加
- `onAuthStateChange`ハンドラーにも同様のエラーハンドリング追加
- メインコンテナを**先に表示**してからデータ取得（真っ白防止）
- エラー発生時は必ずログイン画面を表示するフォールバック
- セッション取得、担当情報取得、初期化処理それぞれにtry-catch
